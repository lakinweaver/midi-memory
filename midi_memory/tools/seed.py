"""Generate demo recordings so the UI can be exercised without a piano.
    ...
    python -m midi_memory.tools.seed --count 14
...
from midi_memory.client.midi.mock_source import SCALES
from midi_memory.client.midi.recorder import finalize_directory
from midi_memory.server.clients import ClientRegistry
...
def main(argv: list[str] | None = None) -> int:
    ...
    # ever going to authenticate as it.
    registry = ClientRegistry(db)
    demo = db.get_client_by_name(DEMO_CLIENT_NAME)
    if demo is None:
        demo, _secret = registry.create(DEMO_CLIENT_NAME)

    # finalize_directory reads the capture thresholds, which live on the client.
    capture = ClientSettings(data_dir=settings.data_dir, min_notes=1, min_seconds=0.5)
    ...
    for i in range(args.count):
        ...
        record = finalize_directory(
            directory, capture, events=events, started_at=started,
            device_name="Yamaha P-125 (demo)",
        )
        if record is None:
            continue

        db.insert_session(SessionUpload.from_record(record),
                          name=rng.choice(NAMES), client_id=demo["id"])
        if rng.random() < 0.75:
