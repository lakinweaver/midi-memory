...
# Paths reachable without a session cookie. Ingest is on this list because
# capture clients are machines, not browsers: they authenticate with their own
# bearer secret, checked by the ingest router itself.
PUBLIC_PATHS = frozenset({"/login", "/logout", "/healthz"})
PUBLIC_PREFIXES = ("/static/", "/api/ingest/")
