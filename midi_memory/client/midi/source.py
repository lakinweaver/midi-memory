...
def create_source(settings: Settings) -> MidiSource:
    ...
    if choice == "mock":
        from midi_memory.client.midi.mock_source import MockSource
        return MockSource(settings)
    if choice == "none":
        return NullSource(settings)
    if choice == "alsa":
        from midi_memory.client.midi.alsa_source import AlsaSource
        return AlsaSource(settings)
    if choice == "portable":
        from midi_memory.client.midi.portable_source import PortableSource
