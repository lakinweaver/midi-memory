...
The link to the server is editable for the same reason: a Raspberry Pi should be
set up from a browser, not from an editor over SSH.
"""
...
EDITABLE = (
    ...
    "marker_enabled",
    "marker_note",
    "marker_double_press_seconds",
    "server_url",
    "client_secret",
    "client_name",
)

# Never echoed back to the browser once set: the page shows whether a secret is
# present, not what it is.
SECRET_KEYS = frozenset({"client_secret"})
...
class SettingsStore:
    ...
    def save(self, changes: dict[str, Any]) -> dict:
        ...
        # This file holds the client secret, so it is not world-readable.
        try:
    ...
    def current(self) -> dict:
        """The editable settings, with the secret reduced to 'is one set?'."""
        out = {}
        for key in EDITABLE:
            value = getattr(self.settings, key)
            out[key] = bool(value) if key in SECRET_KEYS else value
        return out
    ...
    def read_only(self) -> dict:
        ...
        return {
            ...
            "keep_uploaded_days": s.keep_uploaded_days,
